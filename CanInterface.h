...
using ReceiveFunctionType = uint32_t(*)(uint8_t*, uint32_t*, uint32_t);
...
class CanInterface
{
    ...
    uint32_t SendMessage(uint8_t* buffer, uint32_t length, uint32_t destination); 
    uint32_t ReceiveMessage(uint8_t* bufer, uint32_t* length, uint32_t source);
