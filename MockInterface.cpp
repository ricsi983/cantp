...
uint32_t MockInterface::Send(uint8_t* buffer, uint32_t length, uint32_t id)
{
...
std::vector<std::vector<uint8_t>> MockInterface::GetTxMessages(uint32_t id)
{
    return  _txFrames.at(id);
}

 void MockInterface::SetSendResult(uint32_t result)
{
    _sendResult = result;
}

void MockInterface::SetReceiveResult(uint32_t result)
{
    _receiveResult = result;
}

void MockInterface::SetTimeout(bool timeout)
{
    _timeout = timeout;
}

void MockInterface::ClearTxMessages(uint32_t id)
{
    _txFrames.at(id).clear();
}

void MockInterface::ClearRxMessages(uint32_t id)
{
    _rxFrames.at(id).clear();
}

void MockInterface::AddMessageToRx(uint32_t id, uint8_t* buffer, uint8_t length)
{
    std::vector<uint8_t> frame;
    frame.resize(length);
    memcpy(&frame[0], buffer, length);
...
uint32_t MockInterface::Receive(uint8_t* buffer, uint32_t* length, uint32_t id)
{
